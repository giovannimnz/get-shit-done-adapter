...
- **Global scope**: `/home/ubuntu/docker/AtiusCapital/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/home/ubuntu/docker/AtiusCapital/.claude/notes/`)
...
2. Glob `/home/ubuntu/docker/AtiusCapital/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/home/ubuntu/docker/AtiusCapital/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/home/ubuntu/docker/AtiusCapital/.claude/notes/` — works in any directory
