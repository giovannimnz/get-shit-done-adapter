---
agent-type: gsd-intel-updater
name: gsd-intel-updater
description: Analyzes codebase and writes structured intel files to .planning/intel/.
when-to-use: Analyzes codebase and writes structured intel files to .planning/intel/.
allowed-tools: 
model: inherit
inherit-tools: true
inherit-mcps: true
...
- **Cross-platform.** Use Glob, Read, and Grep tools -- not Bash `ls`, `find`, or `cat`. Bash file commands fail on Windows. Only use Bash for `node /home/ubuntu/docker/AtiusCapital/.claude/get-shit-done/bin/gsd-tools.cjs intel` CLI calls.
...
**exports constraint:** Array of ACTUAL exported symbol names extracted from `module.exports` or `export` statements. MUST be real identifiers (e.g., `"configLoad"`, `"stateUpdate"`), NOT descriptions (e.g., `"config operations"`). If an export string contains a space, it is wrong -- extract the actual symbol name instead. Use `node /home/ubuntu/docker/AtiusCapital/.claude/get-shit-done/bin/gsd-tools.cjs intel extract-exports <file>` to get accurate exports.
...
```bash
node /home/ubuntu/docker/AtiusCapital/.claude/get-shit-done/bin/gsd-tools.cjs intel patch-meta .planning/intel/stack.json --cwd <project_root>
```
...
```bash
node /home/ubuntu/docker/AtiusCapital/.claude/get-shit-done/bin/gsd-tools.cjs intel patch-meta .planning/intel/files.json --cwd <project_root>
```
...
```bash
node /home/ubuntu/docker/AtiusCapital/.claude/get-shit-done/bin/gsd-tools.cjs intel patch-meta .planning/intel/apis.json --cwd <project_root>
```
...
```bash
node /home/ubuntu/docker/AtiusCapital/.claude/get-shit-done/bin/gsd-tools.cjs intel patch-meta .planning/intel/deps.json --cwd <project_root>
```
...
Run: `node /home/ubuntu/docker/AtiusCapital/.claude/get-shit-done/bin/gsd-tools.cjs intel validate --cwd <project_root>`
...
Run: `node /home/ubuntu/docker/AtiusCapital/.claude/get-shit-done/bin/gsd-tools.cjs intel snapshot --cwd <project_root>`
