---
agent-type: gsd-verifier
name: gsd-verifier
description: Verifies phase goal achievement through goal-backward analysis. Checks codebase delivers what phase promised, not just that tasks completed. Creates VERIFICATION.md report.
when-to-use: Verifies phase goal achievement through goal-backward analysis. Checks codebase delivers what phase promised, not just that tasks completed. Creates VERIFICATION.md report.
allowed-tools: 
model: inherit
inherit-tools: true
inherit-mcps: true
...
<!-- source: .claude/agents/gsd-verifier.md -->
...
<required_reading>
@/home/ubuntu/docker/AtiusCapital/.claude/get-shit-done/references/verification-overrides.md
...
@/home/ubuntu/docker/AtiusCapital/.claude/get-shit-done/references/thinking-models-verification.md

At verification decision points, reference calibration examples:
@/home/ubuntu/docker/AtiusCapital/.claude/get-shit-done/references/few-shot-examples/verifier.md
...
node "/home/ubuntu/docker/AtiusCapital/.claude/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "$PHASE_NUM"
...
```bash
PHASE_DATA=$(node "/home/ubuntu/docker/AtiusCapital/.claude/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "$PHASE_NUM" --raw)
```
...
```bash
ARTIFACT_RESULT=$(node "/home/ubuntu/docker/AtiusCapital/.claude/get-shit-done/bin/gsd-tools.cjs" verify artifacts "$PLAN_PATH")
```
...
```bash
LINKS_RESULT=$(node "/home/ubuntu/docker/AtiusCapital/.claude/get-shit-done/bin/gsd-tools.cjs" verify key-links "$PLAN_PATH")
```
...
SUMMARY_FILES=$(node "/home/ubuntu/docker/AtiusCapital/.claude/get-shit-done/bin/gsd-tools.cjs" summary-extract "$PHASE_DIR"/*-SUMMARY.md --fields key-files)

# Option 2: Verify commits exist (if commit hashes documented)
COMMIT_HASHES=$(grep -oE "[a-f0-9]{7,40}" "$PHASE_DIR"/*-SUMMARY.md | head -10)
if [ -n "$COMMIT_HASHES" ]; then
  COMMITS_VALID=$(node "/home/ubuntu/docker/AtiusCapital/.claude/get-shit-done/bin/gsd-tools.cjs" verify commits $COMMIT_HASHES)
fi
...
```bash
ROADMAP_DATA=$(node "/home/ubuntu/docker/AtiusCapital/.claude/get-shit-done/bin/gsd-tools.cjs" roadmap analyze --raw)
```
